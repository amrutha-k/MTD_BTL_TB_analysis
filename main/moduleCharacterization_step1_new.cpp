  ...
  for (int irun=0;irun<tree.size();++irun)
	...
	for(unsigned int iBar = 0; iBar < channelMapping.size()/2; ++iBar){
	  ...
	  if (channelIdx[chL[iBar]] >=0 && channelIdx[chR[iBar]] >=0){
		...
		phi_L[iBar] = (timeL[iBar]-floor(timeL[iBar]/6250.)*6250)/6250.;
		phi_R[iBar] = (timeR[iBar]-floor(timeR[iBar]/6250.)*6250)/6250.;
	...
	for(unsigned int iBar = 0; iBar < channelMapping.size()/2; ++iBar)                                                                                                                                  
	  {
		  ...
		  )
		{
		  ...
		  anEvent.phi_L = phi_L[iBar];
		  anEvent.phi_R = phi_R[iBar];
		  if(useTrackInfo){
		    anEvent.nhits = nhits;
		    anEvent.x = x;
		    anEvent.y = y;
		  ...
		  if(useMCP && iev_H4DAQ>-1){
			...
			anEvent.phi_MCP = (anEvent.t_MCP - anEvent.t_CLK_C - 0.849*6.25 - floor((anEvent.t_MCP - anEvent.t_CLK_C)/6.25)*6.25)/6.25;
			anEvent.phi_MCP_C = anEvent.phi_MCP - round(anEvent.phi_MCP);
		  }
		  else{
		    anEvent.amp_MCP = -999;
		    anEvent.t_MCP = -999;
