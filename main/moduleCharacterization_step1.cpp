...
int main(int argc, char** argv)
{
  ...
  tree -> SetBranchStatus("mh_n",    1); tree -> SetBranchAddress("mh_n",   &mh_n);
  ...
  //-------------- to find the bar and beam posotions -------------
  TH2F *hbeam = new TH2F("hbeam","hbeam; x (mm); y (mm)", 100, -50, 50, 100, -50, 50);	  
  TH2F *h_bar8_pos = new TH2F("h_bar8_pos","h_bar8_pos; x (mm); y (mm)", 80, -40, 40, 80, -40, 40); 
  TH2F *h_bar3_pos = new TH2F("h_bar3_pos","h_bar3_pos; x (mm); y (mm)", 80, -40, 40, 80, -40, 40);
  ...
  if( !opts.GetOpt<std::string>("Coincidence.status").compare("yes") &&
	...
	if (!opts.GetOpt<std::string>("Input.sourceName").compare("TB")){
	    ...
	    if (energySumArray > 1000. ) continue;
	    if (nBarsArray > 5 ) continue;
	} 

	if (channelIdx[chL_ext] <0 || channelIdx[chR_ext] <0) continue;   //coincidence cut
	...
	if ( totL_ext < 0 && totL_ext > 100 && totR_ext < 0 && totR_ext > 100 ) continue;
	...
	if( h1_energyLR_ext[index] == NULL ){
	  ...
	  //h1_totL_ref[index] = new TH1F(Form("h1_tot_refbarL_Vov%.2f_th%02.0f",Vov,vth),"",500,0.,100.);
	  //h1_totR_ref[index] = new TH1F(Form("h1_tot_refbarR_Vov%.2f_th%02.0f",Vov,vth),"",500,0.,100.);
	...
	//h1_totL_ref[index] -> Fill(totL_ext);
	//h1_totR_ref[index] -> Fill(totR_ext);  
      }
	...
	for( auto index : h1_energyLR_ext){
	  ...
	  if( opts.GetOpt<int>("Channels.array") == 1){
	    ...
	    index.second->GetXaxis()->SetRangeUser(Vov*30.,700);   //Vov*20. 
	  ...
	  f_pre -> SetRange(max*0.75, max*2.0);  //.8, 1.5
	  ...
	  if (f_pre->GetParameter(1)>10)
	    rangesLR[index.first] -> push_back( 0.75*f_pre->GetParameter(1));   //0.8
	  else
	    rangesLR[index.first] -> push_back( 20 );
	  rangesLR[index.first] -> push_back( 940. );   //700.
  ...
  for(int entry = 0; entry < nEntries; ++entry) {
    ...
    //--------- for spacial uniformity studies -------------
    //if (x < -3 || x > -1) continue;
    //if (x < -1 || x > 1) continue;
    //if (x < 1 || x > 3) continue;
    //if (x < 3 || x > 5) continue;
    ...
    //if (x < 11 || x > 13) continue;
    //if (x < -7 || x > 16) continue;
    ...
    if(!opts.GetOpt<std::string>("Coincidence.status").compare("yes"))
      {
	if(!acceptEvent[entry] ) continue;    // ------- coincidence check between 2 channels in the reference module
	...
	if ( (!opts.GetOpt<std::string>("Input.sourceName").compare("TB")) && ( avEn < rangesLR[label]-> at(0) || avEn > rangesLR[label]-> at(1) ) ) {
	  continue;     // -------- selection on energy range of the external bar
	}
      }
	...
	int index_ref( (10000*int(Vov*100.)) + (100*vth) + 99);
    ...
    if( ( thrZero.GetThresholdZero(chL[iBar],vthMode) + vth) > 63. ) continue;
	  ...
	  if (!opts.GetOpt<std::string>("Input.sourceName").compare("TB") && (energySumArray > 940 || nBarsArray > 5)) continue; // to remove showering events
	...
	//----------- to find the bar positions -----------------
	  if (iBar == 8 && energyL[iBar] > 467 && energyR[iBar] > 325){

		  h_bar8_pos->Fill(x,y);
	  }

	  if (iBar == 3 && energyL[iBar] > 129 && energyR[iBar] > 369){
  ...
  TCanvas *c_test = new TCanvas();
	  c_test->cd();
	  hbeam->Draw("colz");
	  c_test->SaveAs("/eos/user/a/amkrishn/www/MTDBTL_TB_OCT2021_CERN/run4894/test/beam.png");
	  c_test->Clear();

	  c_test->cd();
	  h_bar8_pos->Draw("colz");
	  c_test->SaveAs("/eos/user/a/amkrishn/www/MTDBTL_TB_OCT2021_CERN/run4894/test/bar8.png");
	  c_test->Clear();

	  c_test->cd();
	  h_bar3_pos->Draw("colz");
	  c_test->SaveAs("/eos/user/a/amkrishn/www/MTDBTL_TB_OCT2021_CERN/run4894/test/bar3.png");
	  c_test->Clear();
