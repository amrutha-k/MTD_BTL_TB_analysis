...
int main(int argc, char** argv)
{
  ...
  while( std::getline(ss,token,',') )
  {
    ...
    for(int run = runMin; run <= runMax; ++run)
    {
      ...
      //std::string fileName = Form("%s/%s%04d_ped_t.root",inputDir.c_str(),fileBaseName.c_str(),run);
  ...
  for(int entry = 0; entry < nEntries; ++entry)
  {
    ...
    for(auto ch : channels)
    {
      ...
      else
      {
        ...
        std::string channelR = opts.GetOpt<std::string>(Form("%s.channelR",ch.c_str()));
        int chIDR = opts.GetOpt<int>(Form("%s.chID",channelR.c_str()));      
        
        float qfineL  = qfine[int(chIDL)];
        float totL    = tot[int(chIDL)]/1000.;
        float energyL = energy[int(chIDL)];
        ...
        if( qfineR < cut_qfineAcc[chIDR][step1] ) continue;
        if( totR < cut_totAcc[chIDR][step1] ) continue;
        
        h1_energy[label] -> Fill( energy );        
      }
    }
