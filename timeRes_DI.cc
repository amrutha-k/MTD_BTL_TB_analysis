...
/*  vector<double> mean_;
  vector<double> meanerr_;
    ...
    vector<int> xbins_a{1,12,16,21,29,50};
    vector<int> xbins_p{18,31,36,39,42,45};

    vector<double> xvalues(int a)
     {
     ...
     if (a == 0){
      ...
      for(int i = 0; i<xbins_a.size()-1; i++){
        ...
        x = (0.7+xbins_a[i]*0.023) + (0.7+xbins_a[i+1]*0.023);
...
int main(int argc, char* argv[]){
  ...
  std::vector<double> y_mean,x_arr_pos,x_arr_amp;

  Long64_t nentries = mytree->GetEntries();
  
  for (int ith=15; ith < 151; ith+= 5) {
    ...
    TH2D *tdiff_vs_ampSIPM0 = new TH2D(Form("tdiff_vs_ampSIPM0_th%d",ith), "; amp/MPV; t0 - t1 (ns)", 100, 0.7, 3, 200, -0.5, 0.5);
    TH2D *tdiff_vs_ampSIPM1 = new TH2D(Form("tdiff_vs_ampSIPM1_th%d",ith), "; amp/MPV; t0 - t1 (ns)", 100, 0.7, 3, 200, -0.5, 0.5);
    ...
    TH2D *t0_minus_tMCP_vs_amp = new TH2D(Form("t0_minus_tMCP_vs_amp_th%d",ith), "; amp (mV); t0 - tMCP (ns)", 100, 0.0003, 0.0009, 100, -55.8, -54.6);
    TH2D *t1_minus_tMCP_vs_amp = new TH2D(Form("t1_minus_tMCP_vs_amp_th%d",ith), "; amp (mV); t1 - tMCP (ns)", 100, 0.0003, 0.0009, 100, -55.6, -54.6);
    TH2D *t0_minus_tMCP_vs_amp_corrected = new TH2D(Form("t0_minus_tMCP_vs_amp_corrected_th%d",ith), "; amp (mV); t0 - tMCP (ns)", 100, 0.0003, 0.0009, 100, -0.5, 0.5);
    TH2D *t1_minus_tMCP_vs_amp_corrected = new TH2D(Form("t1_minus_tMCP_vs_amp_corrected_th%d",ith), "; amp (mV); t1 - tMCP (ns)", 100, 0.0003, 0.0009, 100, -0.5, 0.5);

    TF1 *fit_xCorrection = new TF1("fit_xCorrection","pol1",26,28);
    TF1 *fit_xCorrection1 = new TF1("fit_xCorrection1","pol1",26,28);
    TF1 *fit_ampwalk0 = new TF1("fit_ampwalk0","pol3",0.00033,0.0009);
    TF1 *fit_ampwalk1 = new TF1("fit_ampwalk1","pol3",0.00033,0.0009);
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //cuts applied
        ...
        if(event.ampLo[0]< 1075. || event.ampLo[0] > 3000) continue;
        if(event.ampLo[1]< 1108. || event.ampLo[1] > 3000) continue;
    //    if(event.ampLo[0]< 1075. || event.ampLo[0] > 4605) continue;
     //   if(event.ampLo[1]< 1108. || event.ampLo[1] > 4740) continue;
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      tdiff_vs_ampSIPM0->Fill(event.ampLo[0]/1536, t0_AmpCorrected[jentry] - t1_AmpCorrected[jentry]);
      tdiff_vs_ampSIPM1->Fill(event.ampLo[1]/1583, t0_AmpCorrected[jentry] - t1_AmpCorrected[jentry]);
    ...
    TGraphErrors *gr_tdiff_vs_amp0 = new TGraphErrors();
    TGraphErrors *gr_tdiff_vs_amp1 = new TGraphErrors();
    x_arr_amp = xvalues(0);

    for(int i = 0; i<xbins_a.size()-1; i++){

        TH1D *t_sipm0 = tdiff_vs_ampSIPM0->ProjectionY("",xbins_a[i],xbins_a[i+1]);
        ...
        c->Clear();
        gr_tdiff_vs_amp0 ->SetPoint(gr_tdiff_vs_amp0->GetN(), x_arr_amp[i], f_sipm0->GetParameter(1));
        gr_tdiff_vs_amp0 ->SetPointError(gr_tdiff_vs_amp0->GetN()-1, 0, f_sipm0->GetParError(1));   
        
        gr_tdiff_vs_amp1 ->SetPoint(gr_tdiff_vs_amp1->GetN(), x_arr_amp[i], f_sipm0->GetParameter(2)/2);
        gr_tdiff_vs_amp1 ->SetPointError(gr_tdiff_vs_amp1->GetN()-1, 0, f_sipm0->GetParError(2)/2);  
    ...
    gr_tdiff_vs_amp0->SetMarkerSize(1);
    gr_tdiff_vs_amp0->SetMarkerStyle(20);
    gr_tdiff_vs_amp0->SetMarkerColor(kBlack);
    gr_tdiff_vs_amp0->SetTitle("mean of t0 - t1 vs slices in amp; amp/MPV ; t0 - t1 mean");
    gr_tdiff_vs_amp0->Draw("AP same");
    c->SaveAs(Form("/eos/user/a/amkrishn/www/MTD_BTL_TB_FNAL_APRIL-MAY2021/46076/c_tdiff_vs_amp0_with_graph_th%d_%s.png",ith,argv[1]));
    c->Clear();
    ...
    gr_tdiff_vs_amp1->SetTitle("time resolution vs slices in amp; amp/MPV ; (t0 - t1)/2");
    gr_tdiff_vs_amp1->Draw("APL");
    c->SaveAs(Form("/eos/user/a/amkrishn/www/MTD_BTL_TB_FNAL_APRIL-MAY2021/46076/c_tdiff_vs_amp0_sigma_th%d_%s.png",ith,argv[1]));
    c->Clear();

    //fout->cd();
    //gr_tdiff_vs_amp0->Write("gr_tdiff_vs_amp0_mean");
    //gr_tdiff_vs_amp1->Write("gr_tdiff_vs_amp0_sigma");

    delete gr_tdiff_vs_amp0, gr_tdiff_vs_amp1;
...
if(ith == 50 || ith == 60){
    ...
    for(int i = 0; i<xbins_p.size()-1; i++){
        ...
        c->cd();
        t_->Draw();
        c->SaveAs(Form("/eos/user/a/amkrishn/www/MTD_BTL_TB_FNAL_APRIL-MAY2021/46076/c_gaus_pos_test_th%d_%s_%d.png",ith,argv[1],i));
        c->Clear();
    ...
    c->SaveAs(Form("/eos/user/a/amkrishn/www/MTD_BTL_TB_FNAL_APRIL-MAY2021/46076/c_tdiff_vs_x_corrected_mean_th%d_%s.png",ith,argv[1]));
    c->Clear(); 
    ...
    gr_tdiff_vs_x_sigma->Draw("APL");
    c->SaveAs(Form("/eos/user/a/amkrishn/www/MTD_BTL_TB_FNAL_APRIL-MAY2021/46076/c_tdiff_vs_x_corrected_sigma_th%d_%s.png",ith,argv[1]));
    c->Clear(); 
  ...
  */
    ...
    delete htRes_diff_AmpXposCorrected, tavg_vs_x_ampCorrected, tavg_vs_x_ampCorrected;
    delete l0, l1, l2, l3;
    //delete gr_tdiff_vs_x_mean;
    delete fit_tRes_diff1, fit_tRes_diff2, fit_tRes_diff3, fit_tRes_diff, fit_tRes_diff4, fit_tRes_diff5, fit_tRes_diff6, fit_tRes_diff7,fit_tRes_diff8, fit_xCorrection1;
