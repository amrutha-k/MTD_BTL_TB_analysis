...
int main(int argc, char** argv)
{
  ...
  TFile* outFile = new TFile(Form("%s/pulseShape_%s.root", plotDir.c_str(), outName.c_str()),"RECREATE");
  ...
  for(auto mapIt : h1_time1_totSel_chL)
    {
      ...
      //--- to avoid the outliers which are mostly at thr = 3 dac
      if(ith == 3) continue;
  ...
  //std::string plotDir(Form("/eos/user/a/amkrishn/www/MTDBTL_TB_OCT2021_CERN/run4894/refBar8/pulseShapes/%s",outName.c_str()));
  ...
  {  
    ...
    for(int point1 = 0; point1 <  g_pulseShapeL[index2]->GetN()-npoints; ++point1)
    {
      ...
      if( f_temp->GetParameter(1) > slewRate )
      {
        ...
        fitFuncL -> SetParError(0,f_temp->GetParError(0));
        fitFuncL -> SetParError(1,f_temp->GetParError(1));
    ...
    fitFuncLowL -> SetParError(0, f_temp->GetParError(0));
    fitFuncLowL -> SetParError(1, f_temp->GetParError(1));
    ...
    fitFuncL -> Draw("same");
    TLatex* latexL = new TLatex(0.30,0.80,Form("slew rate max = %.1f #pm %.1f #muA/ns",fitFuncL->GetParameter(1),fitFuncL->GetParError(1)));
    latexL -> SetNDC();
    latexL -> SetTextFont(82);
    latexL -> SetTextSize(0.033);
    latexL -> SetTextAlign(11);
    ...
    TLatex* latexL_low = new TLatex(0.30,0.70,Form("slew rate timing th. = %.1f #pm %.1f #muA/ns",fitFuncLowL->GetParameter(1),fitFuncLowL->GetParError(1)));
    latexL_low -> SetNDC();
    latexL_low -> SetTextFont(82);
    latexL_low -> SetTextSize(0.033);
    ...
    fitFuncL ->Write(Form("fitFuncL_bar%02d_Vov%.2f",iBar,Vov));
    fitFuncLowL ->Write(Form("fitFuncLowL_bar%02d_Vov%.2f",iBar,Vov));
    ...
    for(int point1 = 0; point1 <  g_pulseShapeR[index2]->GetN()-npoints; ++point1)
    {
      ...
      if( f_temp->GetParameter(1) > slewRate )
      {
        ...
        fitFuncR -> SetParError(0, f_temp->GetParError(0));
        fitFuncR -> SetParError(1, f_temp->GetParError(1));
    ...
    fitFuncLow_ch2 -> SetParError(0, f_temp->GetParError(0));
    fitFuncLow_ch2 -> SetParError(1, f_temp->GetParError(1));
    ...
    fitFuncR -> Draw("same");
    TLatex* latexR = new TLatex(0.30,0.76,Form("slew rate max = %.1f #pm %.1f #muA/ns",fitFuncR->GetParameter(1),fitFuncR->GetParError(1)));
    latexR -> SetNDC();
    latexR -> SetTextFont(82);
    latexR -> SetTextSize(0.033);
    latexR -> SetTextAlign(11);
    ...
    TLatex* latexR_low = new TLatex(0.30,0.66,Form("slew rate timing th. = %.1f #pm %.1f #muA/ns",fitFuncLow_ch2->GetParameter(1),fitFuncLow_ch2->GetParError(1)));
    latexR_low -> SetNDC();
    latexR_low -> SetTextFont(82);
    latexR_low -> SetTextSize(0.033);
    ...
    fitFuncR ->Write(Form("fitFuncR_bar%02d_Vov%.2f",iBar,Vov));
    fitFuncLow_ch2 ->Write(Form("fitFuncLowR_bar%02d_Vov%.2f",iBar,Vov));
  }
