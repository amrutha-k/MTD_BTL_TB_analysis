...
int main(int argc, char** argv)
{
  ...
  for(auto mapIt : trees)
      ...
      for(int entry = 0; entry < nEntries; ++entry)
	{
	  ...
	  if(anEvent->energyL > 0 && anEvent->energyR > 0 && anEvent->energyL < 940 && anEvent->energyR < 940) h2_energyL_vs_energyR[index1] -> Fill (anEvent->energyL, anEvent->energyR);
  ...
  for(auto stepLabel : stepLabels)
      ...
      for(int iBar = 0; iBar < 16; ++iBar) 
	{
	  ...
	  // -- draw energy L vs energy R
	      ...
	      fitFunc_energyL_vs_energyR[index1] = new TF1(Form("fitFunc_energyL_vs_energyR_%s",labelLR.c_str()),"pol1",ranges["L-R"][index1]->at(0),ranges["L-R"][index1]->at(0)+420);
